...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun  4 15:56:36 2018
...
def process(args):
    ...
    with open("./data/test_user_ratings.dat") as fin:
        fin.next()
        groundtruth = [line.strip().split("\t")[:3] for line in fin]    # (user, profile, rating)
...
def predict_rating(model, nodedict, user, movie):
    """
    Predicts the rating between a user and a profile by finding the profile-rating node with the highest
    similarity to the given user node.
    Loops through the ten possible profile-rating nodes and finds the node with the highest similarity to the user.
    
    Returns an integer rating 1-10.
    """
    user_id = nodedict[user].id
    max_sim = -10
    rating = 1
    for i in range(1,10):
        p_rating_id = nodedict[p+ '_'+str(i)].id
        # print(user_id + ' ' + p_rating_id)
        sim = model.similarity(user_id,p_rating_id)
        if sim > max_sim:
            max_sim = sim
            rating = i
    # print max_sim        
